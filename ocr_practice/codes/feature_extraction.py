# Pytorch package를 사용하여 신경망 구성
# 구성한 신경망을 통해 문자 이미지에서 특성 추출 함수 및 class 생성
import torch.nn as nn
import torch.nn.functional as F

# Visual Geometric Group; VGG : 다중 레이어가 있는 표준 심층 CNN 아키텍쳐
# 해당 아키텍쳐에서의 deep(깊이)는 구성하는 CNN의 개수로, 주로 16, 19개로 구성된 VGG-16, VGG-19 등이 쓰임
# VGG architecture는 객체 인식 모델이자, 이미지 인식 아키텍쳐 중 하나
# 은닉층으로 ReLU를 사용,
class VGG_FeatureExtractor(nn.Module):
    """ FeatureExtractor of CRNN (https://arxiv.org/pdf/1507.05717.pdf) """

    # channel : 3차원 tensor = [height, width, color]
...
class RCNN_FeatureExtractor(nn.Module):
    ...
    # GRCL : Gated Recurrent Convolution Layer
    def __init__(self, input_channel, output_channel=512):
